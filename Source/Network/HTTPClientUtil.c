...
err_t HTTPClientHeaderPrintFN(__unused httpc_state_t *connection, __unused void *arg, struct pbuf *hdr, u16_t hdr_len, __unused u32_t content_len) {
    HTTP_INFO("\nheaders %u\n", hdr_len);
    u16_t offset = 0;
...
err_t HTTPClientReceivePrintFN(__unused void *arg, __unused struct altcp_pcb *conn, struct pbuf *p, err_t err) {
    HTTP_INFO("\ncontent err %d\n", err);
    u16_t offset = 0;
...
static err_t InternaHeaderFN(httpc_state_t *connection, void *arg, struct pbuf *hdr, u16_t hdr_len, u32_t content_len) {
    assert(arg);
    HTTPRequest_t *req = (HTTPRequest_t*)arg;
    if (req->headers_fn) {
...
static err_t InternaRecvFN(void *arg, struct altcp_pcb *conn, struct pbuf *p, err_t err) {
    assert(arg);
    HTTPRequest_t *req = (HTTPRequest_t*)arg;
    if (req->recv_fn) {
...
static void InternalResultFN(void *arg, httpc_result_t httpc_result, u32_t rx_content_len, u32_t srv_res, err_t err) {
    assert(arg);
    HTTPRequest_t *req = (HTTPRequest_t*)arg;
...
int HTTPClientRequestAsync(async_context_t *context, HTTPRequest_t *req) {
#if LWIP_ALTCP
...
#endif
    req->complete = false;
    req->settings.headers_done_fn = req->headers_fn ? InternaHeaderFN : NULL;
    req->settings.result_fn = InternalResultFN;
    async_context_acquire_lock_blocking(context);
    err_t ret = httpc_get_file_dns(req->hostname, req->port ? req->port : default_port, req->url, &req->settings, InternaRecvFN, req, NULL);
...
int HTTPClientRequestSync(async_context_t *context, HTTPRequest_t *req) {
    assert(req);
    int ret = HTTPClientRequestAsync(context, req);
    if (ret != 0) {
        return ret;
    }
    while(!req->complete) {
